...
내부 압축 파일은 현재 저장소 검색상 확인되지 않았다.

대신 누락된 숏츠 5개를 `shared/presets/9x16/batch-001-010/`에 실제 config 구조로 추가했다.

추가 파일:

```txt
popwifi-mv-studio/shared/presets/9x16/batch-001-010/shorts-001-midnight-jazz-frame/config.json
popwifi-mv-studio/shared/presets/9x16/batch-001-010/shorts-002-lofi-window-glow/config.json
popwifi-mv-studio/shared/presets/9x16/batch-001-010/shorts-003-acoustic-spring-card/config.json
popwifi-mv-studio/shared/presets/9x16/batch-001-010/shorts-004-kpop-neon-stage/config.json
popwifi-mv-studio/shared/presets/9x16/batch-001-010/shorts-005-ballad-memory-film/config.json
```

기존 9:16 `moon-dust-window` 1개도 남겨두었다. 따라서 현재 9:16 batch-001-010에는 최소 6개 config가 존재한다.
...
4. 숏츠 프리셋 페이지에서 5개 추가 프리셋이 보이는지 확인
5. 롱폼/숏츠 프리셋 선택 테스트
