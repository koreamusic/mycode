...
function buildFrame() {
  const c='#8888c0';return `<defs><linearGradient id="fg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="${c}" stop-opacity="0.80"/><stop offset="100%" stop-color="${c}" stop-opacity="0.48"/></linearGradient><filter id="sf"><feGaussianBlur stdDeviation="0.4"/></filter><filter id="glow"><feGaussianBlur stdDeviation="1.8" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
    <rect x="24" y="24" width="852" height="458" stroke="${c}" stroke-width="2" fill="none" opacity="0.60" stroke-dasharray="12 8"/>
    <rect x="34" y="34" width="832" height="438" stroke="${c}" stroke-width="0.8" fill="none" opacity="0.28" stroke-dasharray="4 6"/>
    ${[[24,24],[876,24],[24,482],[876,482]].map(([x,y])=>`<circle cx="${x}" cy="${y}" r="4" fill="${c}" opacity="0.72"/>`).join('')}`;
}

function buildParticles(l) {
  l.innerHTML='';for(let i=0;i<12;i++){const el=document.createElement('div');const s=2+Math.random()*3,x=Math.random()*100,y=Math.random()*100,dur=1.5+Math.random()*2.5,del=-(Math.random()*dur);el.style.cssText=`position:absolute;left:${x}%;top:${y}%;pointer-events:none;`;el.innerHTML=`<svg width="${s*3}" height="${s*3}"><circle cx="${s*1.5}" cy="${s*1.5}" r="${s}" fill="rgba(136,136,192,0.5)"><animate attributeName="opacity" values="0.1;0.6;0.1" dur="${dur}s" begin="${del}s" repeatCount="indefinite"/></circle></svg>`;l.appendChild(el);}
}

function buildEq() {
  const svgW=100,svgH=48,n=8,durs=[0.36,0.52,0.42,0.58,0.34,0.50,0.44,0.60];let svg=`<svg width="${svgW}" height="${svgH}" viewBox="0 0 ${svgW} ${svgH}">`;for(let i=0;i<n;i++){const x=4+i*12,h=14+i%4*9,an=`ds${_ID()}`;svg+=`<style>@keyframes ${an}{0%{transform:scaleY(0.1);}100%{transform:scaleY(1);}}</style><line x1="${x+4}" y1="${svgH}" x2="${x+4}" y2="${svgH-h}" stroke="rgba(136,136,192,0.68)" stroke-width="2" stroke-dasharray="4 3" style="animation:${an} ${durs[i]}s ease-in-out infinite alternate;transform-origin:${x+4}px ${svgH}px;transform-box:fill-box;"/>`;}return svg+'</svg>';
}
