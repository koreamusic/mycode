// Clean Night Minimal 16x9 — 클린 나이트 미니멀
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Clean Night Minimal', W / 2, H * 0.44);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(200,210,230,0.0)'; ctx.strokeStyle = 'rgba(200,210,230,0.48)';
    ctx.lineWidth = Math.max(1, W * 0.0012);
    roundRect(ctx, sx, sy, chipW, chipH, H * 0.012); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#e8ecf0'; ctx.textAlign = 'center';
...
function drawBottomBar(ctx, W, H, data) {
  ...
  ctx.fillStyle = 'rgba(0,0,0,0)'; ctx.strokeStyle = 'rgba(200,210,230,0.28)';
  ctx.lineWidth = Math.max(1, W * 0.0012);
  ctx.beginPath(); ctx.moveTo(bX, bY); ctx.lineTo(bX + bW, bY); ctx.stroke();
  ctx.fillStyle = '#e8ecf0'; ctx.textAlign = 'center';
  ctx.font = Math.floor(W * 0.020) + "px 'Noto Sans KR', sans-serif";
  ctx.fillText(data.currentLyric?.text || data.song?.title || 'Clean Night Minimal', W / 2, bY + bH * 0.58);
  ctx.restore();
}

function drawVisSlot(ctx, x, y, w, h, t, accentColor) {
  ctx.save(); ctx.strokeStyle = accentColor + '66'; ctx.lineWidth = Math.max(1, w * 0.010);
  const cx = x + w / 2; const len = h * (0.28 + Math.abs(Math.sin(t * 0.75)) * 0.12);
...
const meta = { id: 'intro-040-clean-night-minimal', name: 'Clean Night Minimal', ratio: '16x9', accentColor: '#c8d2e6' };
