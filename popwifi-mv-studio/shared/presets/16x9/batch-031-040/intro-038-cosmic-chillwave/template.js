// Cosmic Chillwave 16x9 — 코스믹 드리미, 신스 스타
function draw(ctx, frame, fps, data) {
  const t = frame / fps;
  const W = ctx.canvas.width; const H = ctx.canvas.height;
  drawBackground(ctx, W, H, t);
  drawStarFrame(ctx, W, H, t);
...
function drawBackground(ctx, W, H, t) {
  ...
  grad.addColorStop(0, '#080418'); grad.addColorStop(0.5, '#100824'); grad.addColorStop(1, '#060418');
  ctx.fillStyle = grad; ctx.fillRect(0, 0, W, H);
  const pulse = 0.10 + Math.sin(t * 1.2) * 0.03;
  const g1 = ctx.createRadialGradient(W * 0.40, H * 0.42, 0, W * 0.40, H * 0.42, H * 0.38);
  g1.addColorStop(0, 'rgba(120,40,220,' + pulse + ')'); g1.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = g1; ctx.fillRect(0, 0, W, H);
  const g2 = ctx.createRadialGradient(W * 0.65, H * 0.40, 0, W * 0.65, H * 0.40, H * 0.30);
  g2.addColorStop(0, 'rgba(0,200,240,' + pulse * 0.7 + ')'); g2.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = g2; ctx.fillRect(0, 0, W, H);
  ctx.save(); ctx.globalAlpha = 0.5;
  ctx.fillStyle = '#ffffff';
  for (let i = 0; i < 30; i++) {
    const sx = (Math.sin(i * 127.3) * 0.5 + 0.5) * W;
    const sy = (Math.cos(i * 83.1) * 0.5 + 0.5) * H;
    ctx.fillRect(sx, sy, Math.max(0.8, W * 0.001), Math.max(0.8, W * 0.001));
  }
  ctx.restore();
}

function drawStarFrame(ctx, W, H, t) {
  ctx.save();
  const p = Math.min(1, t / 1.0);
  ctx.strokeStyle = 'rgba(120,60,240,' + (0.42 + p * 0.25) + ')';
  ctx.lineWidth = Math.max(1.5, W * 0.002);
  roundRect(ctx, W * 0.08, H * 0.08, W * 0.84, H * 0.68, H * 0.03); ctx.stroke();
  ctx.strokeStyle = 'rgba(0,200,240,' + (0.18 * p) + ')';
...
function drawSocial(ctx, W, H, t) {
  ...
  ctx.fillStyle = 'rgba(8,4,24,0.80)'; ctx.strokeStyle = 'rgba(120,60,240,0.50)';
...
function drawBottomBar(ctx, W, H, data) {
  ...
  ctx.fillStyle = 'rgba(8,4,24,0.88)'; ctx.strokeStyle = 'rgba(120,60,240,0.28)';
  ctx.lineWidth = Math.max(1, W * 0.0015);
  roundRect(ctx, bX, bY, bW, bH, H * 0.025); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#f0e8ff'; ctx.textAlign = 'center';
  ctx.font = Math.floor(W * 0.020) + "px 'Noto Sans KR', sans-serif";
  ctx.fillText(data.currentLyric?.text || data.song?.title || 'Cosmic Chillwave', W / 2, bY + bH * 0.58);
  ctx.restore();
}
...
const meta = { id: 'intro-038-cosmic-chillwave', name: 'Cosmic Chillwave', ratio: '16x9', accentColor: '#783cf0' };
