...
function buildFrame() {
  const c='#a89060';const seed=42;const torn=(x1,y1,x2,y2,vert)=>{const pts=[];const n=14,r=()=>(Math.sin(seed++)*10000%1);const len=vert?y2-y1:x2-x1;for(let i=0;i<=n;i++){const t=i/n;const tx=vert?x1+(r()-0.5)*12:x1+t*(x2-x1);const ty=vert?y1+t*(y2-y1):y1+(r()-0.5)*8;pts.push(`${tx},${ty}`);}return `<polyline points="${pts.join(' ')}" stroke="${c}" stroke-width="2.5" fill="none" opacity="0.60" stroke-linecap="round" stroke-linejoin="round"/>`;};return `<defs><linearGradient id="fg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="${c}" stop-opacity="0.85"/><stop offset="100%" stop-color="${c}" stop-opacity="0.50"/></linearGradient><filter id="sf"><feGaussianBlur stdDeviation="0.7"/></filter><filter id="glow"><feGaussianBlur stdDeviation="2" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>${torn(24,24,876,24,'h')}${torn(24,482,876,482,'h')}${torn(24,24,24,482,'v')}${torn(876,24,876,482,'v')}`;
}

function buildParticles(l) {
  l.innerHTML='';for(let i=0;i<5;i++){const el=document.createElement('div');const w=20+Math.random()*25,x=Math.random()*100,dur=10+Math.random()*8,del=-(Math.random()*dur),op=0.06+Math.random()*0.08;el.style.cssText=`position:absolute;left:${x}%;top:-10px;opacity:0;animation:hjF${i} ${dur}s ${del}s linear infinite;pointer-events:none;`;el.innerHTML=`<style>@keyframes hjF${i}{0%{opacity:0;}12%{opacity:${op};}88%{opacity:${op};}100%{opacity:0;transform:translateY(540px) rotate(${(Math.random()-0.5)*15}deg);}}</style><svg width="${w}" height="10" viewBox="0 0 ${w} 10"><path d="M0,5 Q${w*0.25},2 ${w*0.5},5 Q${w*0.75},8 ${w},5" fill="rgba(168,144,96,0.35)" stroke="rgba(168,144,96,0.2)" stroke-width="0.8"/></svg>`;l.appendChild(el);}
}

function buildEq() {
  return eqBase(7,14,48,(x,H,i)=>{const c=`hsl(${35+i*5},50%,${38+i*3}%)`;const pts=`${x-5},${H} ${x-4},${H-H*0.3} ${x-5},${H-H*0.6} ${x-3},${H} ${x},${H-H-3} ${x+3},${H} ${x+5},${H-H*0.6} ${x+4},${H-H*0.3} ${x+5},${H}`;return `<polygon points="${pts}" fill="${c}" opacity="0.72"/>`;});
}
