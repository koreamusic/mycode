// Golden Hour Folk 16x9 — 황금빛 시간, 오픈 에어 포크
function draw(ctx, frame, fps, data) {
  const t = frame / fps;
  const W = ctx.canvas.width; const H = ctx.canvas.height;
  drawBackground(ctx, W, H, t);
  drawGoldenFrame(ctx, W, H, t);
...
function drawBackground(ctx, W, H, t) {
  ...
  grad.addColorStop(0, '#f8f0d8'); grad.addColorStop(0.5, '#fef4d0'); grad.addColorStop(1, '#f0e8c0');
  ctx.fillStyle = grad; ctx.fillRect(0, 0, W, H);
  const golden = ctx.createRadialGradient(W * 0.80, H * 0.10, 0, W * 0.80, H * 0.10, H * 0.55);
  golden.addColorStop(0, 'rgba(255,200,80,0.28)'); golden.addColorStop(1, 'rgba(255,255,255,0)');
  ctx.fillStyle = golden; ctx.fillRect(0, 0, W, H);
}

function drawGoldenFrame(ctx, W, H, t) {
  ctx.save();
  const p = Math.min(1, t / 1.0);
  ctx.strokeStyle = 'rgba(160,110,40,' + (0.42 + p * 0.20) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Golden Hour Folk', W / 2, H * 0.44);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(42,26,6,0.10)'; ctx.strokeStyle = 'rgba(160,110,40,0.52)';
    ctx.lineWidth = Math.max(1, W * 0.0015);
    roundRect(ctx, sx, sy, chipW, chipH, H * 0.035); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#2a1a06'; ctx.textAlign = 'center';
...
function drawBottomBar(ctx, W, H, data) {
  ...
  ctx.fillStyle = 'rgba(42,26,6,0.82)'; ctx.strokeStyle = 'rgba(160,110,40,0.30)';
  ctx.lineWidth = Math.max(1, W * 0.0015);
  roundRect(ctx, bX, bY, bW, bH, H * 0.025); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#f8f0d8'; ctx.textAlign = 'center';
  ctx.font = Math.floor(W * 0.020) + "px 'Noto Serif KR', serif";
  ctx.fillText(data.currentLyric?.text || data.song?.title || 'Golden Hour Folk', W / 2, bY + bH * 0.58);
  ctx.restore();
}
...
const meta = { id: 'intro-033-golden-hour-folk', name: 'Golden Hour Folk', ratio: '16x9', accentColor: '#a06e28' };
