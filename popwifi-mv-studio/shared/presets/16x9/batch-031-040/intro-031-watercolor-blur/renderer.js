...
function buildFrame() {
  const c='#7898d0';const blob=(cx,cy,rx,ry,rot,op)=>`<ellipse cx="${cx}" cy="${cy}" rx="${rx}" ry="${ry}" fill="${c}" opacity="${op}" transform="rotate(${rot},${cx},${cy})" filter="url(#wc_bl)"/>`;const corner=(tx,ty,sx,sy)=>`<g transform="translate(${tx},${ty}) scale(${sx},${sy})">${blob(30,30,40,22,-20,0.20)}${blob(50,14,22,12,-35,0.14)}<path d="M0,90 L0,0 L90,0" stroke="${c}" stroke-width="2" fill="none" opacity="0.52"/></g>`;return `<defs><linearGradient id="fg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="${c}" stop-opacity="0.85"/><stop offset="100%" stop-color="${c}" stop-opacity="0.52"/></linearGradient><filter id="sf"><feGaussianBlur stdDeviation="0.7"/></filter><filter id="glow"><feGaussianBlur stdDeviation="2.2" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter><filter id="wc_bl"><feGaussianBlur stdDeviation="6"/></filter></defs>${corner(26,26,1,1)}${corner(874,26,-1,1)}${corner(26,480,1,-1)}${corner(874,480,-1,-1)}<rect x="24" y="24" width="852" height="458" stroke="${c}" stroke-width="1.5" fill="none" opacity="0.42"/>`;
}

function buildParticles(l) {
  l.innerHTML='';for(let i=0;i<8;i++){const el=document.createElement('div');const s=15+Math.random()*20,x=Math.random()*100,y=Math.random()*80,h=190+Math.random()*50,dur=5+Math.random()*5,del=-(Math.random()*dur);el.style.cssText=`position:absolute;left:${x}%;top:${y}%;pointer-events:none;`;el.innerHTML=`<svg width="${s}" height="${s*0.6}"><ellipse cx="${s/2}" cy="${s*0.3}" rx="${s/2-1}" ry="${s*0.28}" fill="hsl(${h},55%,62%)" opacity="0.08"><animate attributeName="opacity" values="0.04;0.12;0.04" dur="${dur}s" begin="${del}s" repeatCount="indefinite"/></ellipse></svg>`;l.appendChild(el);}
}

function buildEq() {
  const n=8,durs=[0.32,0.48,0.38,0.55,0.34,0.50,0.42,0.58];let svg=`<svg width="104" height="48" viewBox="0 0 104 48">`;for(let i=0;i<n;i++){const x=4+i*13,h=14+i%4*9,an=`wc${_ID()}`;svg+=`<style>@keyframes ${an}{0%{transform:scaleY(0.15);}100%{transform:scaleY(1);}}</style><ellipse cx="${x+5}" cy="48" rx="5.5" ry="${h/2}" fill="hsl(${200+i*8},55%,58%)" opacity="0.72" style="animation:${an} ${durs[i]}s ease-in-out infinite alternate;transform-origin:${x+5}px 48px;transform-box:fill-box;"/>`;}return svg+'</svg>';
}
