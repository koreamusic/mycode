...
function buildFrame() {
  const c='#e8b820';
    // 악보 오선 + 골든 코너
    const staff=`<g opacity="0.32">${[0,1,2,3,4].map(i=>`<line x1="140" y1="${28+i*7}" x2="760" y2="${28+i*7}" stroke="${c}" stroke-width="0.7"/>`).join('')}${[0,1,2,3,4].map(i=>`<line x1="140" y1="${476-i*7}" x2="760" y2="${476-i*7}" stroke="${c}" stroke-width="0.7"/>`).join('')}</g>`;
    const corner=(tx,ty,sx,sy)=>`<g transform="translate(${tx},${ty}) scale(${sx},${sy})" filter="url(#sf)">
      <path d="M0,72 L0,0 L72,0" stroke="${c}" stroke-width="3" fill="none" opacity="0.82"/>
      <path d="M10,62 L10,10 L62,10" stroke="${c}" stroke-width="1" fill="none" opacity="0.4"/>
      <circle cx="0" cy="0" r="7" fill="${c}" opacity="0.85"/>
      <circle cx="72" cy="0" r="3.5" fill="${c}" opacity="0.55"/>
      <circle cx="0" cy="72" r="3.5" fill="${c}" opacity="0.55"/>
    </g>`;
    return rectFrame(c,'rgba(230,190,40,0.32)',staff+corner(26,26,1,1)+corner(874,26,-1,1)+corner(26,480,1,-1)+corner(874,480,-1,-1));
}

function buildParticles(l) {
  l.innerHTML='';const n8=['♩','♪','♫','𝄞'];for(let i=0;i<14;i++){const el=document.createElement('div');const s=11+Math.random()*13,x=Math.random()*100,dur=6+Math.random()*8,del=-(Math.random()*dur),sx=(Math.random()-0.5)*70,op=0.12+Math.random()*0.22;el.style.cssText=`position:absolute;left:${x}%;top:-25px;font-size:${s}px;color:#e8b820;opacity:${op};animation:tpF${i} ${dur}s ${del}s linear infinite;pointer-events:none;`;el.innerHTML=`<style>@keyframes tpF${i}{0%{opacity:0;}8%{opacity:${op};}88%{opacity:${op};}100%{opacity:0;transform:translateY(540px) translateX(${sx}px);}}</style>${n8[i%4]}`;l.appendChild(el);}
}

function buildEq() {
  const svgW=100,svgH=48,n=8,durs=[0.28,0.44,0.36,0.52,0.32,0.48,0.40,0.56];let svg=`<svg width="${svgW}" height="${svgH}" viewBox="0 0 ${svgW} ${svgH}"><defs><linearGradient id="tpg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#ffe060"/><stop offset="100%" stop-color="#c89010"/></linearGradient></defs>`;
    for(let i=0;i<n;i++){const x=4+i*12,h=14+i%4*9,an=`tp${_ID()}`;svg+=`<style>@keyframes ${an}{0%{transform:scaleY(0.2);}100%{transform:scaleY(1);}}</style><rect x="${x}" y="${svgH-h}" width="9" height="${h}" rx="2" fill="url(#tpg)" opacity="0.82" style="animation:${an} ${durs[i]}s ease-in-out infinite alternate;transform-origin:bottom;transform-box:fill-box;"/>`;}
    return svg+'</svg>';
}
