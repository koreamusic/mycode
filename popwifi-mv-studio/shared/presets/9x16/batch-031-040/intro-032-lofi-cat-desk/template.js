...
function drawBackground(ctx, W, H, t) {
  ...
  const glow = ctx.createRadialGradient(W*0.65, H*0.28, 0, W*0.65, H*0.28, H*0.32);
  glow.addColorStop(0, 'rgba(240,190,80,0.16)'); glow.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(160,120,220,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Lofi Cat Desk', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(160,120,220,0.52)';
...
const meta = { id: 'intro-032-lofi-cat-desk', name: 'Lofi Cat Desk', ratio: '9x16', accentColor: '#a078dc' };
