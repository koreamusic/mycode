...
function drawBackground(ctx, W, H, t) {
  ...
  g1.addColorStop(0, 'rgba(255,60,160,{p})'.replace('{p}', pulse.toFixed(2))); g1.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(255,80,180,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Pink Stage Pop', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(255,80,180,0.52)';
...
const meta = { id: 'intro-034-pink-stage-pop', name: 'Pink Stage Pop', ratio: '9x16', accentColor: '#ff3ca0' };
