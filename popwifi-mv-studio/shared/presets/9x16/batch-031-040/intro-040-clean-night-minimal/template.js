...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(200,210,230,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Clean Night Minimal', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(200,210,230,0.52)';
...
const meta = { id: 'intro-040-clean-night-minimal', name: 'Clean Night Minimal', ratio: '9x16', accentColor: '#c8d2e6' };
