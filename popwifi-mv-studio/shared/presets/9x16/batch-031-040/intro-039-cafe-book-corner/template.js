...
function drawBackground(ctx, W, H, t) {
  ...
  const glow = ctx.createRadialGradient(W*0.30, H*0.35, 0, W*0.30, H*0.35, H*0.35);
  glow.addColorStop(0, 'rgba(200,160,80,0.14)'); glow.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(180,140,80,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Cafe Book Corner', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(180,140,80,0.52)';
...
const meta = { id: 'intro-039-cafe-book-corner', name: 'Cafe Book Corner', ratio: '9x16', accentColor: '#b48c50' };
