...
function drawBackground(ctx, W, H, t) {
  ...
  const glow = ctx.createRadialGradient(W*0.78, H*0.18, 0, W*0.78, H*0.18, H*0.28);
  glow.addColorStop(0, 'rgba(200,220,255,0.15)'); glow.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(180,200,240,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Moonlit Ballad Frame', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(180,200,240,0.52)';
...
const meta = { id: 'intro-031-moonlit-ballad-frame', name: 'Moonlit Ballad Frame', ratio: '9x16', accentColor: '#b4c8f0' };
