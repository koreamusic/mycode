...
function drawBackground(ctx, W, H, t) {
  ...
  const bloom = ctx.createRadialGradient(W*0.80, H*0.10, 0, W*0.80, H*0.10, W*0.55);
  bloom.addColorStop(0, 'rgba(255,200,80,0.28)'); bloom.addColorStop(1, 'rgba(255,255,255,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(160,110,40,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Golden Hour Folk', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(160,110,40,0.52)';
...
const meta = { id: 'intro-033-golden-hour-folk', name: 'Golden Hour Folk', ratio: '9x16', accentColor: '#a06e28' };
