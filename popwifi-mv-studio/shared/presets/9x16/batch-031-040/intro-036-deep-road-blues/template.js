...
function drawBackground(ctx, W, H, t) {
  ...
  const glow = ctx.createRadialGradient(W*0.50, H*0.65, 0, W*0.50, H*0.65, H*0.38);
  glow.addColorStop(0, 'rgba(100,70,40,0.15)'); glow.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(40,70,160,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillStyle = '#c8c8c0'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.font = Math.floor(W * 0.055) + "px 'Noto Serif KR', serif";
  ctx.fillText(data.song?.title || 'Deep Road Blues', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(40,70,160,0.52)';
...
const meta = { id: 'intro-036-deep-road-blues', name: 'Deep Road Blues', ratio: '9x16', accentColor: '#2846a0' };
