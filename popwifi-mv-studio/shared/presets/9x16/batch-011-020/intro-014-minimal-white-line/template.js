...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(240,240,240,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Minimal White Line', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(240,240,240,0.52)';
...
const meta = { id: 'intro-014-minimal-white-line', name: 'Minimal White Line', ratio: '9x16', accentColor: '#e8e8e8' };
