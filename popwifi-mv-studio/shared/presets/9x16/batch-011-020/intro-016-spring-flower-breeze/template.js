...
function drawBackground(ctx, W, H, t) {
  ...
  const bloom = ctx.createRadialGradient(W*0.65, H*0.22, 0, W*0.65, H*0.22, W*0.30);
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(240,140,170,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Spring Flower Breeze', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(240,140,170,0.52)';
...
const meta = { id: 'intro-016-spring-flower-breeze', name: 'Spring Flower Breeze', ratio: '9x16', accentColor: '#f08caa' };
