...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(180,210,240,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Rainy Piano Window', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(180,210,240,0.52)';
...
const meta = { id: 'intro-011-rainy-piano-window', name: 'Rainy Piano Window', ratio: '9x16', accentColor: '#b4d2f0' };
