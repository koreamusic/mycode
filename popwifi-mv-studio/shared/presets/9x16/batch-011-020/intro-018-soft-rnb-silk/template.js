...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(200,170,185,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Soft R&B Silk', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(200,170,185,0.52)';
...
const meta = { id: 'intro-018-soft-rnb-silk', name: 'Soft R&B Silk', ratio: '9x16', accentColor: '#c8aab9' };
