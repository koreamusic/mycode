...
function drawBackground(ctx, W, H, t) {
  ...
  const bloom = ctx.createRadialGradient(W*0.5, H*0.38, 0, W*0.5, H*0.38, W*0.38);
  bloom.addColorStop(0, 'rgba(100,80,40,0.08)'); bloom.addColorStop(1, 'rgba(255,255,255,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(100,70,40,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Folk Paper Diary', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(100,70,40,0.52)';
...
const meta = { id: 'intro-019-folk-paper-diary', name: 'Folk Paper Diary', ratio: '9x16', accentColor: '#644628' };
