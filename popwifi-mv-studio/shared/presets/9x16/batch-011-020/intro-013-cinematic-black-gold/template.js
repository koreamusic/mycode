...
function drawBackground(ctx, W, H, t) {
  ...
  const v = ctx.createRadialGradient(W/2, H/2, H*0.12, W/2, H/2, H*0.62);
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(200,170,80,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Cinematic Black Gold', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(200,170,80,0.52)';
...
const meta = { id: 'intro-013-cinematic-black-gold', name: 'Cinematic Black Gold', ratio: '9x16', accentColor: '#c8aa50' };
