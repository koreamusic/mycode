...
function drawBackground(ctx, W, H, t) {
  ...
  const glow = ctx.createRadialGradient(W*0.62, H*0.28, 0, W*0.62, H*0.28, H*0.30);
  glow.addColorStop(0, 'rgba(240,180,80,0.16)'); glow.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(180,140,220,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Vinyl Room Lofi', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(180,140,220,0.52)';
...
const meta = { id: 'intro-012-vinyl-room-lofi', name: 'Vinyl Room Lofi', ratio: '9x16', accentColor: '#b48cdc' };
