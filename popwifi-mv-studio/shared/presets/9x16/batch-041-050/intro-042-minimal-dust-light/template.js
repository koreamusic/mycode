...
function drawBackground(ctx, W, H, t) {
  ...
  const bloom = ctx.createRadialGradient(W*0.72, H*0.22, 0, W*0.72, H*0.22, W*0.45);
  bloom.addColorStop(0, 'rgba(200,190,170,0.20)'); bloom.addColorStop(1, 'rgba(255,255,255,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(60,60,70,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Minimal Dust Light', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(60,60,70,0.52)';
...
const meta = { id: 'intro-042-minimal-dust-light', name: 'Minimal Dust Light', ratio: '9x16', accentColor: '#909096' };
