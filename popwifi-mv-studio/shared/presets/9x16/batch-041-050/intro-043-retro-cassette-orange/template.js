...
function drawBackground(ctx, W, H, t) {
  ...
  const glow = ctx.createRadialGradient(W*0.5, H*0.42, 0, W*0.5, H*0.42, H*0.32);
  glow.addColorStop(0, 'rgba(255,140,40,0.14)'); glow.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(255,140,40,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Retro Cassette Orange', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(255,140,40,0.52)';
...
const meta = { id: 'intro-043-retro-cassette-orange', name: 'Retro Cassette Orange', ratio: '9x16', accentColor: '#ff8c28' };
