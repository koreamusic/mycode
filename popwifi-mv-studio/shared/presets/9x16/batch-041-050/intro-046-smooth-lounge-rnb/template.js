...
function drawBackground(ctx, W, H, t) {
  ...
  glow.addColorStop(0, 'rgba(180,120,160,0.12)'); glow.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(200,160,180,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Smooth Lounge R&B', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(200,160,180,0.52)';
...
const meta = { id: 'intro-046-smooth-lounge-rnb', name: 'Smooth Lounge R&B', ratio: '9x16', accentColor: '#c8a0b4' };
