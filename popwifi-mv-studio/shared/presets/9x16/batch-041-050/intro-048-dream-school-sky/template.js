...
function drawBackground(ctx, W, H, t) {
  ...
  const bloom = ctx.createRadialGradient(W*0.25, H*0.20, 0, W*0.25, H*0.20, W*0.22);
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(80,150,220,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Dream School Sky', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(80,150,220,0.52)';
...
const meta = { id: 'intro-048-dream-school-sky', name: 'Dream School Sky', ratio: '9x16', accentColor: '#50a0e0' };
