...
function drawBackground(ctx, W, H, t) {
  ...
  grad.addColorStop(0, '#02020a'); grad.addColorStop(0.5, '#040410'); grad.addColorStop(1, '#020208');
  ctx.fillStyle = grad; ctx.fillRect(0, 0, W, H);
  ctx.save(); ctx.globalAlpha = 0.14;
  ctx.strokeStyle = 'rgba(180,210,255,0.6)'; ctx.lineWidth = Math.max(0.5, W * 0.0006);
  for (let i = 0; i < 22; i++) {
    const rx = (Math.sin(i*113.7)*0.5+0.5)*W; const ry = (i/22)*H*1.2-H*0.1;
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(200,180,80,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Midnight Cinema Rain', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(200,180,80,0.52)';
...
const meta = { id: 'intro-041-midnight-cinema-rain', name: 'Midnight Cinema Rain', ratio: '9x16', accentColor: '#c8b450' };
