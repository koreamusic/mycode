...
function drawBackground(ctx, W, H, t) {
  ...
  const bloom = ctx.createRadialGradient(W*0.82, H*0.14, 0, W*0.82, H*0.14, W*0.50);
  bloom.addColorStop(0, 'rgba(255,200,80,0.25)'); bloom.addColorStop(1, 'rgba(255,255,255,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(100,70,30,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Country Dust Road', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(100,70,30,0.52)';
...
const meta = { id: 'intro-047-country-dust-road', name: 'Country Dust Road', ratio: '9x16', accentColor: '#64461e' };
