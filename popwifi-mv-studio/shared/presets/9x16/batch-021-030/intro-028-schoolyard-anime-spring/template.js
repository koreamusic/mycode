...
function drawBackground(ctx, W, H, t) {
  ...
  grad.addColorStop(0, '#a0d8f0'); grad.addColorStop(0.5, '#c8eaf8'); grad.addColorStop(1, '#e8f8e8');
  ctx.fillStyle = grad; ctx.fillRect(0, 0, W, H);
  const bloom = ctx.createRadialGradient(W*0.70, H*0.20, 0, W*0.70, H*0.20, W*0.25);
  bloom.addColorStop(0, 'rgba(255,200,220,0.40)'); bloom.addColorStop(1, 'rgba(255,255,255,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(80,160,220,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Schoolyard Anime Spring', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(80,160,220,0.52)';
...
const meta = { id: 'intro-028-schoolyard-anime-spring', name: 'Schoolyard Anime Spring', ratio: '9x16', accentColor: '#50a0dc' };
