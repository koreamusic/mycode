...
function drawBackground(ctx, W, H, t) {
  ...
  grad.addColorStop(0, '#04020a'); grad.addColorStop(0.5, '#08020a'); grad.addColorStop(1, '#030008');
  ctx.fillStyle = grad; ctx.fillRect(0, 0, W, H);
  const glow = ctx.createRadialGradient(W*0.5, H*0.30, 0, W*0.5, H*0.30, H*0.34);
  glow.addColorStop(0, 'rgba(200,160,60,0.18)'); glow.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(200,160,60,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillStyle = '#f5e8c0'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.font = Math.floor(W * 0.055) + "px 'Noto Serif KR', serif";
  ctx.fillText(data.song?.title || 'Sax Club Corner', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(200,160,60,0.52)';
...
const meta = { id: 'intro-021-sax-club-corner', name: 'Sax Club Corner', ratio: '9x16', accentColor: '#c8a03c' };
