...
function drawBackground(ctx, W, H, t) {
  ...
  const bloom = ctx.createRadialGradient(W*0.75, H*0.15, 0, W*0.75, H*0.15, W*0.50);
  bloom.addColorStop(0, 'rgba(255,220,100,0.25)'); bloom.addColorStop(1, 'rgba(255,255,255,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(160,120,60,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Guitar Sunbeam', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(160,120,60,0.52)';
...
const meta = { id: 'intro-023-guitar-sunbeam', name: 'Guitar Sunbeam', ratio: '9x16', accentColor: '#a0783c' };
