...
function drawBackground(ctx, W, H, t) {
  ...
  g1.addColorStop(0, 'rgba(180,80,220,{p})'.replace('{p}', pulse.toFixed(2))); g1.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = g1; ctx.fillRect(0, 0, W, H);
  const g2 = ctx.createRadialGradient(W*0.5, H*0.42, 0, W*0.5, H*0.42, H*0.26);
  g2.addColorStop(0, 'rgba(0,200,220,{p})'.replace('{p}', (pulse*0.6).toFixed(2))); g2.addColorStop(1, 'rgba(0,0,0,0)');
...
function drawFrame(ctx, W, H, t) {
  ...
  ctx.strokeStyle = 'rgba(180,120,240,' + (0.45 + p * 0.22).toFixed(2) + ')';
...
function drawTitle(ctx, W, H, data, t) {
  ...
  ctx.fillText(data.song?.title || 'Tape Haze Chillwave', W / 2, H * 0.40);
  ctx.restore();
}
...
function drawSocial(ctx, W, H, t) {
  ...
  items.forEach((label) => {
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; ctx.strokeStyle = 'rgba(180,120,240,0.52)';
...
const meta = { id: 'intro-029-tape-haze-chillwave', name: 'Tape Haze Chillwave', ratio: '9x16', accentColor: '#b478f0' };
